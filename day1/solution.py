...
def simulate_dial(rotations, start_position=50):
    """Simulate the dial rotations and track positions."""
    ...
    print(f"\nThe dial points at 0 a total of {zero_count} times.")
    print(f"Password: {zero_count}")
    
    return positions, zero_count

def create_dial_visualization(positions, rotations):
    ...
    ax_dial.axis('off')
    ax_dial.set_title('Safe Dial', fontsize=16, fontweight='bold')
    ...
    def update(frame):
        nonlocal zero_count, arrow_patch
...
def create_static_visualization(positions, rotations):
    ...
    plt.suptitle(f'Safe Dial Simulation - Password: {zero_count}', 
...
def main():
    ...
    # Simulate
    positions, zero_count = simulate_dial(rotations)
    ...
    fig_static = create_static_visualization(positions, rotations)
    static_file = Path(__file__).parent / 'dial_static.png'
    fig_static.savefig(static_file, dpi=150, bbox_inches='tight')
    print(f"Static visualization saved to: {static_file}")
    
    # Animated visualization
    fig_anim, anim = create_dial_visualization(positions, rotations)
    anim_file = Path(__file__).parent / 'dial_animation.gif'
    anim.save(anim_file, writer='pillow', fps=1)
